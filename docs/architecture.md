...
5. The Extraction Agent parses structured signals from each document: patient, diagnosis, provider, tests, amounts, dates, and quality issues.
...
The extraction layer is currently a local text/OCR parser with quality scoring. In production, I would replace or augment this with OCR plus a structured LLM extraction adapter that returns validated JSON. The rest of the pipeline would stay the same because it already depends on a stable extracted schema.
...
Extraction errors are captured per document and converted into `DEGRADED` trace events. The system continues with remaining signals and reduces confidence. API-level failures return `MANUAL_REVIEW` instead of crashing.

## Limitations

- Uploaded images/PDFs are represented as OCR text in this prototype.
- The extractor uses deterministic patterns rather than a live OCR/LLM service.
